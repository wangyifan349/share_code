...
def move_files(file_info_list, target_directory):
    ...
    for file_info in file_info_list:
        target_path = os.path.join(target_directory, file_info['name'])
        target_path = get_new_file_name(target_directory, file_info['name'])  # 处理重命名
        try:
            shutil.move(file_info['path'], os.path.join(target_directory, target_path))
...
def start_thread():
    ...
    if not os.path.exists(disk_path):
        messagebox.showerror("错误", "指定的路径不存在，请检查后重试。")
        return
